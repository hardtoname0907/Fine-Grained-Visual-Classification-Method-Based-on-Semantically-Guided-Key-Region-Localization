...
def eval(model, testloader, criterion, status, save_path, epoch):
    ...
    with torch.no_grad():
        for i, data in enumerate(tqdm(testloader)):
            ...
            window_scores, coordinates, raw_logits, local_logits, local_imgs,gcn_logits = model(images, epoch, i, status)
            ...
            # gcn
            pred = gcn_logits.max(1, keepdim=True)[1]
            gcn_correct += pred.eq(labels.view_as(pred)).sum().item()
    ...
    gcn_accuracy = gcn_correct / len(testloader.dataset)
