...
from config import max_checkpoint_num, proposalN, eval_trainset, set, gpu_ids
...
def train(model,
    ...
    for epoch in range(start_epoch + 1, end_epoch + 1):
        ...
        for i, data in enumerate(tqdm(trainloader)):
            ...
            assert images.is_cuda, "Input tensors must be moved to GPU before forwarding!"

            proposalN_windows_score, proposalN_windows_logits, indices, \
            window_scores, _, raw_logits, local_logits, _,gcn_logits = model(images, epoch, i, 'train')
            ...
            gcn_loss = criterion(gcn_logits, labels)

            if epoch < 2:
                total_loss = raw_loss
            else:
                total_loss = raw_loss + local_loss + windowscls_loss+gcn_loss
        ...
        if eval_trainset:
            raw_loss_avg, windowscls_loss_avg, total_loss_avg, raw_accuracy, local_accuracy, local_loss_avg,\
                gcn_loss_avg,gcn_accuracy= eval(model, trainloader, criterion, 'train', save_path, epoch)

            print(
                'Train set: raw accuracy: {:.2f}%, local accuracy: {:.2f}%, gcn accuracy: {:.2f}%'.format(
                    100. * raw_accuracy, 100. * local_accuracy, 100. * gcn_accuracy))
            ...
            with SummaryWriter(log_dir=os.path.join(save_path, 'log'), comment='train') as writer:
                ...
                writer.add_scalar('Train/gcn_loss_avg', gcn_loss_avg, epoch)
                writer.add_scalar('Train/gcn_accuracy', gcn_accuracy, epoch)
        ...
        local_loss_avg,gcn_loss_avg,gcn_accuracy\
            = eval(model, testloader, criterion, 'test', save_path, epoch)

        print(
            'Test set: raw accuracy: {:.2f}%, local accuracy: {:.2f}%, gcn accuracy: {:.2f}%'.format(
                100. * raw_accuracy, 100. * local_accuracy, 100. * gcn_accuracy))
        
        epoch_best_accuracy=max(raw_accuracy,local_accuracy,gcn_accuracy)
        if epoch > end_epoch*0.5 and epoch_best_accuracy > best_accuracy:
            best_accuracy = epoch_best_accuracy
            best_epoch = epoch
            torch.save({
                'epoch': epoch,
                'model_state_dict': model.module.state_dict() if multi_gpu else model.state_dict(), # 'model_state_dict': model.state_dict(),
                'learning_rate': lr,
        ...
        with SummaryWriter(log_dir=os.path.join(save_path, 'log'), comment='test') as writer:
            ...
            writer.add_scalar('Train/gcn_loss_avg', gcn_loss_avg, epoch)
            writer.add_scalar('Test/gcn_accuracy', gcn_accuracy, epoch)
    ...
    print('best accuracy: {:.3f}%'.format(100. * best_accuracy))
